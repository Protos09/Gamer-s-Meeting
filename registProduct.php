...
// DBから商品データを取得($p_idが空でなければgetProductメソッド起動)
$dbFormData = (!empty($p_id)) ? getProduct($_SESSION['user_id'], $p_id) : '';
// 新規登録画面か編集画面か判別用フラグ（DBにデータがあればtrue、なければfalse。文言を変更など）
$edit_flg = (empty($dbFormData)) ? false : true;
// DBからカテゴリデータを取得（categorytableから）
...
if(!empty($_POST)){
  ...
  if(empty($dbFormData)){
    ...
    //最大文字数チェック
    validMaxLenRev($comment, 'comment', 500);
    //未入力チェック
...
?>
  ...
  <body class="page-profEdit page-2colum page-logined">
    ...
    <div id="contents" class="site-width">
      <h1 class="page-title"><?php echo (!$edit_flg) ? 'レビューを投稿する' : 'レビューを編集する'; ?></h1>
      <!-- メイン -->
      <section id="main" >
        <div class="form-container">
          <form action="" method="post" class="form" enctype="multipart/form-data" style="width:100%;box-sizing:border-box;">
            ...
            <label class="<?php if(!empty($err_msg['name'])) echo 'err'; ?>">
              ゲームソフト名<span class="label-require">必須</span>
            ...
            <label style="text-align:left;" class="<?php if(!empty($err_msg['price'])) echo 'err'; ?>">
              ...
              <div class="form-group">
                <input type="text" name="price" style="width:150px" placeholder="/100" value="<?php echo (!empty(getFormData('price'))) ? getFormData('price') : 0; ?>"><span class="option">点</span>
              </div>
            </label>
            <div class="area-msg">
              <?php 
