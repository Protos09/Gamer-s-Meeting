...
define('MSG06r','500文字以内で入力してください');//レビュー用
define('MSG07','エラーが発生しました。しばらく経ってからやり直して下さい。');
define('MSG08','そのEmailは既に登録されています');
define('MSG09','メールアドレスまたはパスワードが違います');//どちらかが正しいことがわかってしまうためこのように書く。
define('MSG10', '電話番号の形式が違います');
define('MSG11', '郵便番号の形式が違います');
define('MSG12', '旧パスワードが違います');
define('MSG13', '旧パスワードと同じです');
define('MSG14', '文字で入力してください');
define('MSG15', '正しくありません');
define('MSG16', '有効期限が切れています');
define('MSG17', '半角数字のみご利用いただけます');
define('SUC01', '旧パスワードと同じです');
...
//例外処理
  try {
    //DBへ接続
    $dbh = dbConnect();
    // SQL文作成 検索で引っかかったレコードがいくつあるかcountする
...
//バリデーション関数（最大文字数チェックレビュー用）
function validMaxLenRev($str, $key, $max = 500){
  if(mb_strlen($str)> $max){
    global $err_msg;
    $err_msg[$key] = MSG06r;
  }
...
//DB接続関数
function dbConnect(){
  $dsn = 'mysql:dbname=game_market;host=localhost;charset=utf8';
  $user = 'root';
  $password = 'root';
  $options = array(
    //SQL実行失敗時はエラーコードのみ設定
    PDO::ATTR_ERRMODE => PDO::ERRMODE_SILENT,
    //デフォルトフェッチモードをカラム名をキーとした連想配列形式に設定
...
function sendMail($from, $to, $subject, $comment){
  ...
  mb_internal_encoding("UTF-8");//内部の日本語をどうエンコーディング（機械が分かる言語へ変換）するかを設定
...
function getFormData($str, $flg = false){
  ...
  if(!empty($dbFormData)){
    ...
    if(!empty($err_msg[$str])){
      //POSTにデータがある場合
      if(isset($_method[$str])){//金額や郵便番号などのフォームで数字や数値の0が入っている場合もあるので、issetを使うこと
